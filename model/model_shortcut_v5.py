...
NUM_STYLE = NUMBER(name='encoder', input=6890 * 3, hidden_encoder=[4096, 1024, 256], latent=LATENT_STYLE,
                   hidden_decoder=None,
                   output=None)
NUM_BONELENGTH = NUMBER(name='encoder', input=23, hidden_encoder=[64, 128, 32], latent=LATENT_BONELENGTH,
                        hidden_decoder=None,
                        output=None)
NUM_DECODER_STAR = NUMBER(name='decoder', latent=LATENT_STYLE + LATENT_BONELENGTH, hidden_encoder=[64, 128, 256],
                          output=300, hidden_decoder=None,
                          input=None)
NUM_DECODER_SMPL = NUMBER(name='decoder', latent=LATENT_STYLE + LATENT_BONELENGTH, hidden_encoder=[64, 128, 64],
                          output=10, hidden_decoder=None,
                          input=None)
...
class CVAE(nn.Module):
    ...
    def add_noise(self, value):
        ...
        x_noised = torch.tensor(temp2.cpu().data.numpy(), dtype=torch.float32, requires_grad=True,
                                device=value.get_device())

        _eps = _eps[:, self.element_idx].unsqueeze(1)
        eps = torch.tensor(_eps.repeat(1, value.shape[1]).cpu().data.numpy(), dtype=torch.float32, requires_grad=True,
                           device=value.get_device())
        return x_noised, eps
    ...
    def calculate_bonelength_both_from_mesh(self, v_mesh):
        ...
        real_bone_length = self.calculate_bonelength_both(joints)  # 23
        bone_length = real_bone_length - 0.373924 * 2.67173

        return bone_length, real_bone_length, joints
    ...
    def calculate_bonelength(self, vertex_list,
        ...
        vertex_list_first = torch.index_select(vertex_list, 1,
                                               torch.tensor(first_idx_list, device=vertex_list.get_device()))
        vertex_list_second = torch.index_select(vertex_list, 1,
                                                torch.tensor(second_idx_list, device=vertex_list.get_device()))
    ...
    def jacobianer_base(self, mu_S, mu_B):
        ...
        _mu_hat_B = torch.tensor(mu_hat_B.cpu().data.numpy(), dtype=torch.float32, requires_grad=True,
                                 device=mu_B.get_device())
        _mu_hat_S = torch.tensor(mu_hat_S.cpu().data.numpy(), dtype=torch.float32, requires_grad=True,
                                 device=mu_S.get_device())
...
def loss_wrapper(x, reconstructed_x, z_mu, z_var, bonelength, reconstructed_bonelength, beta, reconstructed_beta,
    ...
    return loss, {'KLD': KLD, 'RCL_bone': RCL_bone, 'RCL_x': RCL_x, 'RCL_beta': RCL_beta,
                  'RCL_x_euclidean_distance': RCL_x_euclidean_distance}
