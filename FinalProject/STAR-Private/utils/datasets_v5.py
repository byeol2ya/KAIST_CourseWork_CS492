...
class StarBetaBoneLengthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     idx = self.debug
        ret = {key: torch.tensor(self.data[key][idx], dtype=torch.float32) for key in self.data}
        #
        if self.transform:
            ret = self.transform(ret)

        return ret['beta'], self.reference['shapeblendshape'], self.reference['mesh_shape_pos'], self.reference['jointregressor_matrix']