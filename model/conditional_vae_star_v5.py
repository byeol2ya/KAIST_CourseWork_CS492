    ...
    with torch.no_grad():
        ...
        for i, (beta, shapeblendshape, mesh_shape_pos, jointregressor_matrix) in enumerate(test_iterator):
            ...
            for k in range(BATCH_SIZE):
                save_as_obj(model=mesh[0].cpu().data.numpy(),save_path="/Data/MGY/STAR-Private/outputs/",name=TRAINED_TIME+"_" + str(k + BATCH_SIZE * i).zfill(4) + "_before")
                save_as_obj(model=generated_mesh[0].cpu().data.numpy(),save_path="/Data/MGY/STAR-Private/outputs/",name=TRAINED_TIME+"_" + str(k + BATCH_SIZE * i).zfill(4) + "_after")
    test_loss =  test_loss
...
def main():
    ...
    get_obj(model=model,save_path="/Data/MGY/STAR-Private/outputs/",name=TRAINED_TIME+"_new",_beta=original_val,_iterator=train_iterator)
    get_obj(model=model,save_path="/Data/MGY/STAR-Private/outputs/",name=TRAINED_TIME+"_new",_beta=new_val,_iterator=train_iterator)
