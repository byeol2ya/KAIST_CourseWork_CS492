...
class StarBetaBoneLengthDataset(Dataset):
    def __init__(self, path, transform=None, debug=-1):
        self.data = load_npz(path=path)
        self.transform = transform
        self.debug = debug
        self.reference = self.load_reference('./datasmpl/reference.npz')
        # assert (True, "Data type is incorrect('train', 'test', 'validation') or param. datasize_dict is incorrect.")

    #https://sanghyu.tistory.com/19
    def load_reference(self, path='./data/reference.npz') -> dict:
