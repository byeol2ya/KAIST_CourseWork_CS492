...
BATCH_SIZE = 2         # number of data points in each batch
...
def loss_wrapper(x, reconstructed_x, z_mu, z_var, bonelength, reconstructed_bonelength, z_mu_bonelength=None, z_var_bonelength=None):
    loss = calculate_base_loss(x, reconstructed_x, z_mu, z_var)
    loss += calculate_bonelength_loss(bonelength, reconstructed_bonelength, z_mu_bonelength, z_var_bonelength)
...
def train(model,train_iterator,optimizer):
    ...
    for i, (beta, bonelength) in enumerate(train_iterator):
        ...
        if e == N_EPOCHS-1:
            ...
            generated_bonelength_list += [[bonelength, z_bonelength]]
        # loss
        # loss = calculate_base_loss(x, reconstructed_x, z_mu, z_var)
...
def test(model,test_iterator):
    ...
    with torch.no_grad():
        for i, (beta, bonelength) in enumerate(test_iterator):
            ...
            # forward pass
            # x, reconstructed_x, z_mu, z_var, z_bonelength, generated_beta, z_mu_bonelength, z_var_bonelength = model(beta, bonelength)
            x, reconstructed_x, z_mu, z_var, bonelength, generated_beta = model(beta, bonelength)
            loss = loss_wrapper(x, reconstructed_x, z_mu, z_var, bonelength, reconstructed_bonelength, z_mu_bonelength=None, z_var_bonelength=None)
